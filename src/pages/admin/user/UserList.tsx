...
const UserList = () => {
  ...
  const openNotification = (placement: NotificationPlacement, title: string) => {
    api.success({
      message: `Thông báo`,
      description: <Context.Consumer>{({ name }) => `${name} thành công ${title}!`}</Context.Consumer>,
      placement,
    });
  };
  ...
  const filterData = () => {
    ...
    if (searchFilter === 'username') {
      ...
      filteredData = filteredData.filter((item) => item.email.toLowerCase().includes(searchText.toLowerCase()));
    } 
