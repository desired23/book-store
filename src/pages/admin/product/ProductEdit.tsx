...
import { useEditProductMutation, useGetProductByIdQuery } from '../../../api/product';
...
const ProductEdit = () => {
  ...
  useEffect(() => {
    setFields();
    const setImages = book?.images.map((img) => ({
      uid: img.publicId,
  ...
  const onFinish = (values: IBook) => {
    ...
    const filteredImageObjects = imageObjects.filter((item) => item !== null) as { url: string; publicId: string }[];

  values.images = filteredImageObjects;
    editBook(values).unwrap().then((data) => {
