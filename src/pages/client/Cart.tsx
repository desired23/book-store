...
const Cart = () => {
  ...
  const handleRemove = async(id: string) => {
    if (authState.isLoggedIn) {
       ...
       await updateCart({
        items: getCart.items.filter(item=>item.book._id !== id).map(item => ({
          book: item.book?._id,
    ...
    <main className="w-4/5 mx-auto">
      <div className="flex shadow-md my-5">
        <div className="w-3/4 bg-white px-10 py-5">
          ...
          <div className="flex mt-10 mb-5">
            ...
            <h3 className=" text-center text-gray-600 text-xs uppercase w-1/5 ">Số lượng</h3>
            <h3 className=" text-center text-gray-600 text-xs uppercase w-1/5">Giá</h3>
            <h3 className=" text-center text-gray-600 text-xs uppercase w-1/5 ">Tổng tiền</h3>
          </div>
          {getCart?.items.map(itemCart => (
            <div key={itemCart.book?._id} className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
              <div className="flex w-2/5"> {/* product */}
                <div className="w-1/4">
                  <Image className="h-24 w-full" src={itemCart?.book?.images[0].url} />
                </div>
                <div className="flex flex-col justify-between ml-4 flex-grow w-3/4">
                  <span className="font-bold text-sm truncate">{itemCart?.book?.title}</span>
                  <span className="text-red-500 text-xs">Apple</span>
                  <Button onClick={() =>handleRemove(itemCart.book?._id?itemCart.book?._id: "")} className=" hover:text-red-500 text-gray-500 text-xs w-fit">Xóa bỏ</Button>
                </div>
              </div>
          ...
          <Link to={`/products`} className="flex  text-indigo-600 text-sm mt-10">
        ...
        <div id="summary" className="w-1/4 px-8 py-10">
          <div className="flex justify-between mt-10 mb-5">
            ...
            <span className=" text-sm">{getCart?.items.reduce((acc, item) => {
      ...
      <select className="block p-2 text-gray-600 w-full text-sm">
        <option>Standard shipping - $10.00</option>
          ...
          <div className="py-10">
            <label htmlFor="promo" className=" inline-block mb-3 text-sm uppercase">Mã giảm giá</label>
            <input type="text" id="promo" placeholder="Enter your code" className="p-2 text-sm w-full" />
          </div>
          <button className="bg-red-500 hover:bg-red-600 px-5 py-2 text-sm text-white uppercase rounded-md font-sans font-bold">Áp dụng</button>
          <div className="border-t mt-8">
            <div className="flex  justify-between py-6 text-sm uppercase">
              <span>Tổng tiền</span>
              <span>{getCart?.items.reduce((acc, item) => {
                return acc + (item.book?.discount * item.quantity)
              }, 0).toLocaleString("vi-VN")}₫</span>
            </div>
